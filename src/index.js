...
var CLIENT_ID = '3MVG9ZL0ppGP5UrDZvIRRUOvYLWkA4IkaN.P9mD6TDEsqXD3unn4nG3YX7ngOYzGVlSjXO2bbJpeK1sijleZK';
var CLIENT_SECRET = '3476988943166485393';
var USERNAME = 'ming.li@davanti.co.nz';
var PASSWORD = '2016apr!';
...
Salesforce.prototype.intentHandlers = {
  ...
  EventStartIntent: function (intent, session, response){
      handleEventStartIntent(session,response);
  },
  EventSubjectIntent:function (intent, session, response){
      handleEventSubjectIntent(intent,session,response);
  },
  EventDateIntent: function(intent, session, response){
      handleEventStartDateIntent(intent,session,response);
  },
  EventTimeIntent: function(intent, session, response){
      handleEventStartTimeIntent(intent,session,response);
  },
  EventWhoIntent: function(intent, session, response){
      handleEventWhoIntent(intent,session,response);
  },
...
// start a new session to create a lead
function handleEventStartIntent(session, response) {
  var speechOutput = "OK, let's create a new event., What is the subject?";
  response.ask(speechOutput);
}

// continue the session, collect the person's name
function handleEventSubjectIntent(intent, session, response) {
  var speechOutput = "Got it. the subject is, " + intent.slots.Subject.value + "., What is the start date?";
  session.attributes.subject = intent.slots.Subject.value;
  response.ask(speechOutput);
}

// continue the session, collect the person's name
function handleEventStartDateIntent(intent, session, response) {
  session.attributes.date = intent.slots.Date.value;
  var speechOutput = "Got it. the date is, " + moment(session.attributes.date).format('MMMM Do YYYY') + "., What about the time?";
  // var speechOutput = "Got it. What about the time?";
  response.ask(speechOutput);
}

// continue the session, collect the person's name
function handleEventStartTimeIntent(intent, session, response) {
  var speechOutput = "Bingo! I created a new event for  "
    + session.attributes.subject + " on " + moment(session.attributes.date+" "+intent.slots.Time.value).format('MMMM Do YYYY') + " at "+moment(session.attributes.date+" "+intent.slots.Time.value).format('h:m a') ;
  var obj = nforce.createSObject('Event');
  obj.set('Subject', session.attributes.subject);
  obj.set('StartDateTime', moment(session.attributes.date+" "+intent.slots.Time.value).format().replace('+00:00','+12:00'));
  obj.set('EndDateTime', moment(session.attributes.date+" "+intent.slots.Time.value).add(1,'hour').format().replace('+00:00','+12:00'));

  console.log(moment(session.attributes.date+" "+intent.slots.Time.value).format());
...
// collect the company name and create the actual lead
function handleEventWhoIntent(intent, session, response) {
  var speechOutput = "Bingo! I created a new event for  "
    + session.attributes.subject + "with "+intent.slots.who.value+" on " + intent.slots.Date.value + " at "+intent.slots.Time.value ;
  var names = session.attributes.name.split(' ');
  var obj = nforce.createSObject('Lead');
  obj.set('FirstName', names[0]);
  obj.set('LastName', names[1]);
...
function handleMyCalendarRequest(response) {
  ...
  org.authenticate({ username: USERNAME, password: PASSWORD }).then(function(){
    ...
    _.forEach(results.records, function(rec) {
      speechOutput += 'At ' + moment(rec.get('StartDateTime')).tz('Pacific/Auckland').format('h:m a') + ', ' + rec.get('Subject');
